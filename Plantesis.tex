...
\begin{description}
  \item[1.a] Realizar un estudio sobre las herramientas existentes
    relacionadas con un entorno BYOD. % Esto no es un objetivo. Es una
                                % tarea para alcanzarlo. 
  \item[1.b] Comparativa de métricas usadas en la literatura: usos,
    ventajas, e inconvenientes de cada una. %Tampoco.
  \item[1.c] Definición de la métrica y justificación de la misma. % Esto sí lo es, porque repite el objetivo. así que se podría eliminar. 
\end{description}
...
\begin{description}
  \item[2.a] Estudio del arte en técnicas Soft Computing aplicadas a
    BYOD. Este sub objetivo está relacionado con el sub objetivo
    1.a. % Tarea
  \item[2.b] Relación entre la métrica escogida o definida en el
    primer objetivo con las técnicas estudiadas en el objetivo
    2.a. %Tarea. Además, ¿qué se va a hacer con la relación?
  \item[2.c] Justificación de las técnicas escogidas respecto a la
    métrica escogida o definida en el primer objetivo. % Tarea, aunque
...
Desarrollar una metodología que se pueda implementar en sistemas
reales, independientemente de la plataforma, y que esté compuesta de
varios pasos: % Metodología ¿para qué? ¿Cómo caracterizas un sistema
...
\begin{enumerate}
  \item Un adecuado pre-procesamiento de datos para asegurar un buen
    mantenimiento de las bases de datos y máximo rendimiento. % Tarea
  \item Entorno de ejecución completo. % Tarea
  \item Validación de los resultados. % Tarea. Si no están validados
